...
import sys
# read model
#weight = np.load('my_data/model_best.npy')
weight = np.load('my_data/model_best.npy')

TEST_IN = sys.argv[1]
RES = sys.argv[2]
#TEST_IN = 'my_data/test.csv'
#RES = 'result/res_best.csv'
...
test_data = np.column_stack((t_pm10,t_pm25,t_o3,t_wind_dir,t_wind_speed,t_wd_hr,t_ws_hr,t_co,t_pm10**3,t_pm25**3,t_pm25*t_o3))
...
#------------------testing------------------------------------------------------
y_out = np.dot(test_data, weight)
value = 0
ID = 0
finalString = "id,value\n"
for i in range(240) :
    #value = ((np.dot(test_data[i], weight)))# + bias))
    finalString = finalString + "id_" + str(i) + "," + str(round(y_out[i][0])) + "\n"
f = open(RES, "w")
f.write(finalString)
f.close()